...
public class YTRemoveCommand implements CommandExecutor{

	@Override
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		
		if(!sender.hasPermission(Perms.command_ytRemove)){
			sender.sendMessage(Lang.NO_PERMS.toString());
			return false;
		}
		
		if(args.length != 1){
			sender.sendMessage(ChatColor.RED + "Correct usage: /ytremove [playername]");
			return false;
		}
		...
		if(!Data.youtubers.containsKey(uuid)){
			sender.sendMessage(ChatColor.RED + "The player " + playerName + " does not have a youtube channel ID set.");
			return false;
		}
		
		Data.youtubers.remove(uuid);
		
		sender.sendMessage(ChatColor.GREEN + "Successfully removed " + playerName + "'s youtube channel ID.");
